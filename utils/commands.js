...
import { ContactPage } from "../src/po/Pages/ContactPage.js";
...
export async function addProductToCart(page, productName, quantity = 1) {
  ...
  await productsPage.accessToProductDetail(productName);
...
export async function filterByCategory(page, categoryName) {
  const productsPage = new ProductsPage(page);
  await productsPage.clickCategoriesLink();

  const categoryActions = {
    "Hand Tools": () => productsPage.clickHandToolsLink(),
    "Power Tools": () => productsPage.clickPowerToolsLink(),
    Other: () => productsPage.clickOtherLink(),
  };
...
export async function validateEcoBadgesAcrossPagination(page) {
  return validateAcrossPagination(page, async (productsPage) => {
    const ecoBadgesCount = await productsPage.ecoBadges.count();
    const totalProducts = await productsPage.productCards.count();
    const allEco = ecoBadgesCount === totalProducts && totalProducts > 0;
...
export async function changeLanguage(page, languageCode) {
  ...
  await contactPage.changeLanguage(languageCode);
}
