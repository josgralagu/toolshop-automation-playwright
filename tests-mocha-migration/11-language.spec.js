...
import { languageMap } from '../utils/testData.js';
  ...
  describe(`Feature: Language Change on Contact Page [${browserName}]`, function () {
    ...
    for (const langCode of LANGUAGES) {
      it(`Change language to ${langCode}`, async function () {
        ...
        const homeLinkText = await contactPage.homeLink.textContent();
        expect(homeLinkText).to.equal(translations.homeLink);

        const categoriesLinkText = await contactPage.categoriesLink.textContent();
        expect(categoriesLinkText.trim()).to.equal(translations.categoriesLink);

        const contactLinkText = await contactPage.contactLink.textContent();
        expect(contactLinkText).to.equal(translations.contactLink);

        const signInLinkText = await contactPage.signInLink.textContent();
