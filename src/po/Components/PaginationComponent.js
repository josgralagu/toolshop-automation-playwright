...
export class PaginationComponent {
	constructor(page) {
		...
		this.pageNumberLinks = page.locator(
	...
	async clickPreviousPage(timeout = 10000) {
		...
		if (!canNavigate) {
			throw new Error(
				"Previous page button is disabled - cannot navigate"
			)
		}
	...
	async clickPageNumber(pageNumber, timeout = 10000) {
		const pageLink = this.page.locator(
			`.pagination .page-link:has-text("${pageNumber}")`
		)
	...
	async isPaginationVisible(timeout = 3000) {
		try {
			await this.paginationContainer.waitFor({
				state: "visible",
				timeout
			})
			return true
