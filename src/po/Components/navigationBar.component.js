export class NavigationBarComponent {
	constructor(page) {
		...
		this.languageDropdownButton = page.locator(
			'[data-test="language-select"]'
		)
		this.languageOption = (code) =>
			page.locator(`[data-test="lang-${code}"]`)
	...
	async clickPowerToolsLink() {
		...
		await this.page.waitForURL("**/category/power-tools", {
			timeout: 15000
		})
	}
	...
	async changeLanguage(languageCode) {
		...
		await this.page.waitForFunction(
			(expectedLanguage) => {
				...
				return (
					button &&
					button.textContent.trim().toUpperCase() === expectedLanguage
				)
			},
