...
export class BookingService {
  ...
  static getByName(firstname, lastname) {
    return apiClient.get(Endpoints.byName()).set(HeaderSets.accept).query({ firstname, lastname });
  }

  static getByDates(checkin, checkout) {
    return apiClient.get(Endpoints.byDates()).set(HeaderSets.accept).query({ checkin, checkout });
  }

  static create(payload) {
    return apiClient.post(Endpoints.create()).set(HeaderSets.common).send(payload);
  }
