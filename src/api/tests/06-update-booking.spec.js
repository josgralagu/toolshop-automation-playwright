...
import { newBookingPayload, updateBookingPayload } from '../utilities/booking.payloads.js';
...
describe('PUT /booking/{id} - Update a booking with Basic Auth', () => {
  ...
  beforeEach(async () => {
    res = await timed(() => BookingService.updateWithAuth(bookingId, updateBookingPayload));
  });
...
describe('PUT /booking/{id} - Update a booking with a token', () => {
  ...
  beforeEach(async () => {
    res = await timed(() => BookingService.updateWithToken(bookingId, updateBookingPayload));
  });
