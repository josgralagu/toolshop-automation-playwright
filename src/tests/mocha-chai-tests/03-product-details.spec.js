...
import { searchAndSelectProduct } from "../../configs/utils/commands.js"
import { searchProducts } from "../../configs/utils/testData.js"
import {
	initializeBrowser,
	closeBrowser,
	pwExpect
	...
	describe(`Product Details Page [${browserName}]`, function () {
		...
		for (const productName of searchProducts) {
			it(`View details of - ${productName}`, async function () {
				...
				await pwExpect(detailPage.productPrice).toBeVisible({
					timeout: 10000
				})
				await pwExpect(detailPage.productDescription).toBeVisible({
					timeout: 10000
				})
			})
		}
