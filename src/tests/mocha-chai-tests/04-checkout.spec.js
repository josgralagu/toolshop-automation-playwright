import { addProductsAndCollectData, getCartData, calculateExpectedSubtotal, getProductNames, getProductQuantities, getProductPrices, calculateLineTotalsError } from '../../configs/utils/commands.js';
  ...
  describe(`Checkout – Subtotal Calculation [${browserName}]`, function () {
    ...
    cartConfigurations.forEach(({ productCount, quantityPerProduct }) => {
      it(`${productCount} products × ${quantityPerProduct} units`, async function () {
        ...
        expect(cartData.names.sort()).to.deep.equal(getProductNames(products).sort());
        expect(cartData.quantities).to.deep.equal(getProductQuantities(products));
        expect(cartData.prices).to.deep.equal(getProductPrices(products));
