...
test.describe("Filters - Multiple Filters", () => {
	multipleFilters.forEach(({ category, brand }) => {
		test(`Apply ${category} + ${brand} filters`, async ({ page }) => {
			...
			const errors = await validateAndNavigateToProductDetails(
				page,
				category,
				brand,
				categoryKeywords
			...
			expect(
				errors,
				`Filters validation failed for ${category} + ${brand}: ${errors.join(", ")}`
			).toEqual([])

			// Verify product details
			const actualBrand = await verifyProductDetails(page)
			expect(actualBrand, `Brand badge should show "${brand}"`).toBe(
				brand
			)
		})
	})
